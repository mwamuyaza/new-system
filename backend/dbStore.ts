...
// MERN Stack Mongoose Schema and Model for University State persistence
const UniversityStateSchema = new mongoose.Schema({
  name: { type: String, default: "current_state", unique: true },
  data: { type: mongoose.Schema.Types.Mixed, required: true },
}, { timestamps: true });

const UniversityStateModel = mongoose.models.UniversityState || mongoose.model('UniversityState', UniversityStateSchema);
...
export class DbStore {
  ...
  private async connectMongo() {
    ...
    if (!mongoUri) {
      console.log("[MERN SETUP] MONGODB_URI is not set in environment variables. Falling back to local JSON file storage.");
      return;
    }

    this.isConnecting = true;
    try {
      console.log(`[MERN SETUP] Connecting to MongoDB at ${mongoUri}...`);
      await mongoose.connect(mongoUri, {
        serverSelectionTimeoutMS: 5000,
      });
      this.isMongoConnected = true;
      this.isConnecting = false;
      console.log("[MERN SETUP] Successfully connected to MongoDB!");

      // Retrieve state from MongoDB or seed it
      await this.syncStateWithMongo();
    } catch (err: any) {
      this.isConnecting = false;
      this.isMongoConnected = false;
      console.error("[MERN SETUP] MongoDB connection error: ", err.message || err);
      console.log("[MERN SETUP] Falling back to local JSON storage mode.");
    }
  }

  private async syncStateWithMongo() {
    try {
      const doc = await (UniversityStateModel as any).findOne({ name: "current_state" });
      if (doc) {
        console.log("[MERN SETUP] Found existing university state in MongoDB. Synchronizing local state...");
        this.state = doc.data;
      } else {
        console.log("[MERN SETUP] No existing state found in MongoDB. Seeding current local state to MongoDB...");
        const newDoc = new UniversityStateModel({
          name: "current_state",
          data: this.state
        });
        await newDoc.save();
        console.log("[MERN SETUP] Successfully seeded initial university state into MongoDB.");
      }
    } catch (err) {
      console.error("[MERN SETUP] Failed to synchronize state with MongoDB:", err);
    }
  }
  ...
  private saveState(state: DbState): void {
    ...
    if (this.isMongoConnected) {
      (UniversityStateModel as any).updateOne(
        { name: "current_state" },
        { $set: { data: state } },
        { upsert: true }
      ).catch((err: any) => {
        console.error("[MERN SETUP] Failed to save state to MongoDB:", err);
      });
    }
  }
  ...
  public updateSessionActivity(token: string): boolean {
    ...
    if (session) {
      ...
      // Update in-memory only, avoid heavy disk I/O on every API call
      return true;
    }
    return false;
  }
