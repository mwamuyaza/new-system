...
import { sendEmail, simulatedEmailsInbox } from "./mailer.js";
...
// --- DEVELOPER / SIMULATION SANDBOX ENDPOINTS ---
app.get("/api/dev/simulated-emails", (req: Request, res: Response): void => {
  res.json(simulatedEmailsInbox);
});

app.delete("/api/dev/simulated-emails", (req: Request, res: Response): void => {
  simulatedEmailsInbox.length = 0;
...
app.post("/api/admin/email-templates/test-send", authenticateToken, authorizeRole(['ADMIN']), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { to, subject, text, html } = req.body;
  if (!to || !subject || !text || !html) {
    res.status(400).json({ error: "Missing required mail dispatch parameters (to, subject, text, html)" });
    return;
  }
  try {
    const result = await sendEmail({ to, subject, text, html });
    if (result.success && result.provider !== 'SIMULATION') {
      res.json({ message: `Success! Test email successfully sent to ${to} using real SMTP provider (${result.provider}).` });
    } else if (result.provider === 'SIMULATION') {
      res.status(400).json({ error: "Email delivered via simulation fallback because SMTP configuration was not read correctly." });
    } else {
      res.status(500).json({ error: "Delivery attempt completed but failed to route.", result });
    }
  } catch (err: any) {
